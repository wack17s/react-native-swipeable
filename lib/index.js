...
var Swipeable = function (_PureComponent) {
  ...
  function Swipeable() {
    ...
    var _this = _possibleConstructorReturn(this, (Swipeable.__proto__ || Object.getPrototypeOf(Swipeable)).call(this));
    ...
    _this.openLeftButtons = function (animationFn, animationConfig, onDone) {
      _this._autoSwipe(Swipeable.AutoSwipeType.OpenLeftButtons, animationFn, animationConfig, onDone);
    };

    _this.openRightButtons = function (animationFn, animationConfig, onDone) {
      _this._autoSwipe(Swipeable.AutoSwipeType.OpenRightButtons, animationFn, animationConfig, onDone);
    };
    ...
    _this.recenter = function () {
      ...
      _this._autoSwipe(Swipeable.AutoSwipeType.Recenter, animationFn, animationConfig, onDone);
    };

    _this._autoSwipe = function (type, animationFn, animationConfig, onDone) {
      var pan = _this.state.pan;


      var finalAnimationFn = animationFn || _reactNative.Animated.timing;
      var finalAnimationConfig = animationConfig || {
        toValue: _this._getAutoSwipeTargetPoint(type),
        duration: 250,
        easing: _reactNative.Easing.elastic(0.5)
      };

      _this.setState({
        lastOffset: _this._getAutoSwipeTargetPoint(type),
        leftActionActivated: false,
    ...
    _this._handlePanResponderMove = function (event, gestureState) {
      ...
      var dx = _this._invertInRtl(gestureState.dx);
      var vx = _this._invertInRtl(gestureState.vx);
    ...
    var _this$props4 = _this.props,
        onPanAnimatedValueRef = _this$props4.onPanAnimatedValueRef,
  ...
  _createClass(Swipeable, [{
    ...
    key: '_invertInRtl',
    value: function _invertInRtl(value) {
      return this.props.isRTL ? -value : value;
    }
  }, {
    key: '_getTextDirectionDependentValue',
    value: function _getTextDirectionDependentValue(ltrValue, rtlValue) {
      return this.props.isRTL ? rtlValue : ltrValue;
    }
  }, {
    key: '_getAutoSwipeTargetPoint',
    value: function _getAutoSwipeTargetPoint(type) {
      switch (type) {
    ...
    value: function _renderButtons(buttons, isLeftButtons) {
      ...
      var leftEnd = this._getTextDirectionDependentValue(canSwipeLeft ? -width : 0, canSwipeLeft ? 0 : -width);
      var rightEnd = this._getTextDirectionDependentValue(canSwipeRight ? width : 0, canSwipeRight ? 0 : width);
      var inputRange = this._getTextDirectionDependentValue(isLeftButtons ? [0, rightEnd] : [leftEnd, 0], isLeftButtons ? [leftEnd, 0] : [0, rightEnd]);

      return buttons.map(function (buttonContent, index) {
        var outputMultiplier = -index / count;
        var outputRange = _this2._getTextDirectionDependentValue(isLeftButtons ? [0, rightEnd * outputMultiplier] : [leftEnd * outputMultiplier, 0], isLeftButtons ? [leftEnd * outputMultiplier, 0] : [0, rightEnd * outputMultiplier]);
        var transform = [{
    ...
    value: function render() {
      ...
      var transform = [{
        translateX: pan.x.interpolate({
          inputRange: this._getTextDirectionDependentValue([canSwipeLeft ? -width : 0, canSwipeRight ? width : 0], [canSwipeLeft ? 0 : -width, canSwipeRight ? 0 : width]),
          outputRange: this._getTextDirectionDependentValue([canSwipeLeft ? -width + _reactNative.StyleSheet.hairlineWidth : 0, canSwipeRight ? width - _reactNative.StyleSheet.hairlineWidth : 0], [canSwipeLeft ? 0 : -width + _reactNative.StyleSheet.hairlineWidth, canSwipeRight ? 0 : width - _reactNative.StyleSheet.hairlineWidth]),
          extrapolate: 'clamp'
        })
      }];
...
Swipeable.propTypes = {
  ...
  isRTL: _propTypes.PropTypes.bool,
...
Swipeable.AutoSwipeType = Object.freeze({
  Recenter: 'recenter',
